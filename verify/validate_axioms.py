...
def matmul_0(A, B):
    ...
    if len(sa) == len(sb) == 2:
        ...
        require(sa[2] == sb[1])
        require(sa[0] == sb[0])
        C = Tensor([[[sum(A[n, i, k] * B[n, k, j] for k in range(sa[2]))
                      for j in range(sb[2])] for i in range(sa[1])] for n in range(sa[0])])
                        ...
                        for kh in range(sb[2]):
                            for kw in range(sb[3]):
                                ...
                                if posH >= 0 and posH < sa[2] and posW >= 0 and posW < sa[3]:
                                    value += A[n, cin+group_idx*sb[1],
                                               posH, posW] * B[c, cin, kh, kw]
                    C[n, c, h, w] = value if acti == AC_MODE_NONE else _relu(
                        value)
...
def concat_0(d, A, B):
    ...
    C = Tensor.zeros(
        tuple(A.shape[i] if i != d else A.shape[i] + B.shape[i] for i in range(A.dim)))
...
def split_1(d, A):
    ...
    s, l, r = A.splits[d]
    C = Tensor.zeros(
        tuple(A.shape[i] if i != d else A.shape[i] - s for i in range(A.dim)))
    for ii in product(*[range(n) for n in C.shape]):
        jj = list(ii)
        jj[d] += s
        C[ii] = A[tuple(jj)]
...
def enlarge_0(kx, ky, A):
    ...
    # TODO: compute split tree for other dimensions?
    C.splits = (A.splits[0], A.splits[1], (), ())
    return C
...
if __name__ == '__main__':
    ...
    if True:
        ...
        print(now(), "Checking a total of {} combinations...\n".format(
            sum(len(x) if x is not None else 0 for x in spaces)))
            ...
            for s in space:
                if False:
                    ...
                    results.append(pool.apply_async(
                        check_axiom, [s], callback=print_function))
